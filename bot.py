...
DB_DIR = "database"
DB_PATH = os.path.join(DB_DIR, "virtex.db")
...
def ensure_user_in_conn(conn: sqlite3.Connection, user_id: int, ref_id: Optional[str] = None) -> None:
    now = now_ts()
    row = conn.execute("SELECT id, registered_at, ref_id FROM users WHERE id = ?", (str(user_id),)).fetchone()
    if not row:
        conn.execute(
            "INSERT INTO users (id, coins, vix, GGs, lost_coins, won_coins, status, checks, registered_at, ref_id, ref_earned, ref_count) VALUES (?, ?, 0, 0, 0, 0, 0, '[]', ?, ?, 0, 0)",
            (str(user_id), START_BALANCE, now, ref_id),
        )
        if ref_id:
...
def ensure_user(user_id: int, ref_id: Optional[str] = None) -> None:
    conn = get_db()
    try:
        ensure_user_in_conn(conn, user_id, ref_id)
        conn.commit()
    finally:
        conn.close()
...
def create_check_atomic(user_id: int, per_user: float, count: int) -> tuple[bool, str]:
    ...
    try:
        ...
        ensure_user_in_conn(conn, user_id)
        row = conn.execute("SELECT coins FROM users WHERE id = ?", (str(user_id),)).fetchone()
        if float(row["coins"] or 0) < total: conn.rollback(); return False, "Недостаточно средств для создания чека."
...
def claim_check_atomic(user_id: int, code: str) -> tuple[bool, str, float]:
    ...
    try:
        ...
        if not row: conn.rollback(); return False, "Чек не найден.", 0.0
        if int(row["remaining"] or 0) <= 0: conn.rollback(); return False, "Этот чек уже закончился.", 0.0
        claimed = json.loads(row["claimed"] or "[]")
        if str(user_id) in {str(x) for x in claimed}: conn.rollback(); return False, "Ты уже активировал этот чек.", 0.0
        ...
        conn.execute("UPDATE checks SET remaining = remaining - 1, claimed = ? WHERE code = ?", (json.dumps(claimed, ensure_ascii=False), code.upper()))
        conn.commit()
...
def list_my_checks(user_id: int) -> list[sqlite3.Row]:
    conn = get_db()
    try:
        return list(conn.execute("SELECT code, per_user, remaining FROM checks WHERE creator_id = ? ORDER BY rowid DESC LIMIT 10", (str(user_id),)).fetchall())
    finally:
        conn.close()
...
def add_deposit(user_id: int, amount: float, term_days: int) -> tuple[bool, str]:
    ...
    if not ok: return False, "Недостаточно средств для открытия депозита."
    conn = get_db()
    try:
...
def list_user_deposits(user_id: int, active_only: bool = False) -> list[sqlite3.Row]:
    conn = get_db()
    try:
        if active_only: return list(conn.execute("SELECT * FROM bank_deposits WHERE user_id = ? AND status = 'active' ORDER BY id DESC", (str(user_id),)).fetchall())
...
def roulette_roll(choice: str) -> tuple[bool, float, str]:
    ...
    return win, multiplier, f"Выпало {number} ({pretty_color})"

def crash_roll() -> float:
    return round(max(1.0, min(50.0, 0.99 / (1.0 - random.random()))), 2)
...
def mines_multiplier(opened_count: int, mines_count: int) -> float:
    ...
    return round(((9.0 / max(1.0, safe_cells)) ** opened_count) * 0.95, 2)
...
def render_ochko_table(game: Dict[str, Any], reveal_dealer: bool) -> str:
    player_cards, dealer_cards = game["player"], game["dealer"]
    if reveal_dealer: dealer_line = f"{format_hand(dealer_cards)} ({hand_value(dealer_cards)})"
    else: dealer_line = f"{dealer_cards[0][0]}{dealer_cards[0][1]} ??"
    return f"🎴 <b>Очко</b>\nСтавка: <b>{fmt_money(game['bet'])}</b>\n\nДилер: {dealer_line}\nТы: {format_hand(player_cards)} ({hand_value(player_cards)})"
...
def ntower_multiplier(level: int, mines: int, house_edge: float = 0.97, max_mult: float = 10000.0) -> float:
    mines, level = max(1, min(4, int(mines))), max(1, int(level))
    p_single = (5 - mines) / 5.0
    if p_single <= 0: return max_mult
    return round(min((1.0 / (p_single ** level)) * house_edge, max_mult), 2)
...
def mines_kb(game: Dict[str, Any], reveal_all: bool = False) -> InlineKeyboardMarkup:
    opened, mines_set = set(game["opened"]), set(game["mines"])
    rows = []
    for start in (1, 4, 7):
        row = []
...
def check_share_kb(code: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📋 Скопировать", callback_data=f"check:copy:{code}")],
        [InlineKeyboardButton(text="📤 Поделиться", switch_inline_query=code)],
    ])

def admin_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 Выдать dC", callback_data="admin:give:coins")],
...
def get_bot_token() -> str:
    ...
    return BOT_TOKEN# ==================== КОМАНДЫ ОБЩИЕ ====================
...
async def balance_command(message: Message):
    ...
    await message.answer(
        f"{headline_user(EMOJI_COIN, message.from_user.id, message.from_user.first_name, 'твой баланс')}\n"
...
async def profile_command(message: Message):
    ...
    reg_date = fmt_dt(reg_ts)

    # Место в топе
    top_rows = get_top_balances(1000)
    place = "—"
    for idx, row in enumerate(top_rows, start=1):
        if int(row["id"]) == user_id: place = str(idx); break
...
async def bonus_command(message: Message):
    ...
    if now - last < BONUS_COOLDOWN_SECONDS:
        ...
        await message.answer(
            f"{headline_user(EMOJI_BONUS, user_id, message.from_user.first_name, 'ты уже забрал бонус')}\n"
    ...
    await message.answer(
        f"{headline_user(EMOJI_BONUS, user_id, message.from_user.first_name, 'ты получил бонус')}\n"
        f"<blockquote>Начислено: <b>{fmt_money(reward)}</b>\nНовый баланс: <b>{fmt_money(balance)}</b></blockquote>"
    )
...
async def help_command(message: Message):
    ...
    if is_admin_user(message.from_user.id):
        admin_hint = (
            "\n\n🛠️ Админ-команды:\n"
            "• <code>админ</code> — панель управления\n"
            "• <code>выдать 1000</code> (reply) — выдать dC\n"
            "• <code>/new_promo</code> — создать промо\n"
    ...
    await message.answer(
        ...
        "• <code>П сумма</code> (reply) — перевести dC</blockquote>\n\n"
        "Игры:\n"
...
async def ref_command(message: Message):
    ...
    lines = [
        ...
        "·····················",
        f"{EMOJI_BONUS} Приглашайте друзей по своей ссылке и получайте бонусы:",
        ...
        "",
        f"{EMOJI_COIN} Уже заработано:",
        f"⤷ <b>{fmt_money(stats['ref_earned'])}</b>",
        f"{EMOJI_INVITED} Приглашено друзей:",
        f"⤷ <b>{stats['ref_count']} чел.</b>",
        "",
        "ℹ️ Чтобы друг был зачислен, он должен сыграть хотя бы одну игру или собрать бонус.",
    ]
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📋 Скопировать ссылку", callback_data=f"ref:copy:{user_id}")],
        [InlineKeyboardButton(text="📤 Поделиться", switch_inline_query=f"ref_{user_id}")],
    ])
...
async def transfer_start(message: Message, state: FSMContext):
    ...
    if len(parts) < 2:
        await message.answer("Формат: <code>П сумма</code> (reply на сообщение) или <code>П ID сумма</code>")
        return

    # Если reply — берём target из reply
    if message.reply_to_message and message.reply_to_message.from_user:
        target = message.reply_to_message.from_user
        try:
            amount = parse_amount(parts[1])
        except Exception:
            await message.answer("Введи корректную сумму. Пример: <code>П 1000</code>")
            return
        if amount < 10:
            await message.answer("Минимальная сумма перевода: 10 dC")
            return
        await finish_transfer(message, target.id, target.full_name, amount)
        return

    # Иначе — ищем ID/username в аргументах
    if len(parts) >= 3:
        try:
            target_raw = parts[1]
            amount = parse_amount(parts[2])
        except Exception:
            await message.answer("Формат: <code>П ID сумма</code> или <code>П @username сумма</code>")
            return
        if amount < 10:
            await message.answer("Минимальная сумма перевода: 10 dC")
            return
        ...
        target_raw_clean = target_raw.replace("@", "").strip()
        try:
            target_id = int(target_raw_clean)
        ...
        if target_id:
            await finish_transfer(message, target_id, target_name, amount)
        else:
            await message.answer("Укажи ID или username пользователя.")
    else:
        await message.answer("Ответь на сообщение игрока командой <code>П сумма</code> или укажи <code>П ID сумма</code>")

async def finish_transfer(message: Message, to_id: int, to_name: str, amount: float):
    from_id = message.from_user.id
    if from_id == to_id:
        await message.answer("❌ Нельзя перевести самому себе.")
        return
    ok, msg = transfer_coins(from_id, to_id, amount)
    if not ok:
        await message.answer(f"❌ {msg}")
        return
    from_balance = float(get_user(from_id)["coins"] or 0)
    to_balance = float(get_user(to_id)["coins"] or 0)
    await message.answer(
        f"✅ <b>Перевод выполнен</b>\n"
        f"<blockquote>Кому: {mention_user(to_id, to_name)}\n"
        f"Сумма: <b>{fmt_money(amount)}</b>\n"
        f"Твой баланс: <b>{fmt_money(from_balance)}</b></blockquote>"
    )
    try:
        await message.bot.send_message(
            to_id,
            f"{EMOJI_COIN} <b>Получен перевод!</b>\n"
            f"<blockquote>От: {mention_user(from_id, message.from_user.first_name)}\n"
            f"Сумма: <b>{fmt_money(amount)}</b>\n"
            f"Твой баланс: <b>{fmt_money(to_balance)}</b></blockquote>"
        )
    except Exception:
        pass
...
async def admin_command(message: Message):
    if not is_admin_user(message.from_user.id):
        ...
        return
    await message.answer(
        f"{EMOJI_CROWN} <b>Админ-панель</b>\nВыбери действие:",
...
async def admin_give_coins(message: Message):
    ...
    await message.answer(
        f"✅ Выдано <b>{fmt_money(amount)}</b>\n"
        f"Кому: {mention_user(target.id, target.full_name)}\n"
...
async def admin_cb(query: CallbackQuery, state: FSMContext):
    ...
    if action == "give":
        ...
        await query.message.answer("💰 Введи сумму для выдачи dC (reply на сообщение игрока):")
        await query.answer()
    elif action == "stats":
        conn = get_db()
        try:
        ...
        await query.message.answer(
            ...
            f"Ставок всего: <b>{bets_count}</b>\n"
            f"Сумма ставок: <b>{fmt_money(float(total_bet))}</b>\n"
            f"Сумма выплат: <b>{fmt_money(float(total_payout))}</b></blockquote>"
        )
        await query.answer()
    elif action == "top20":
        ...
        if not rows:
            ...
            lines = ["👥 <b>Топ-20 игроков</b>", "<blockquote>"]
            for idx, row in enumerate(rows, start=1):
                lines.append(f"{idx}. {mention_user(int(row['id']))} — <b>{fmt_money(float(row['coins'] or 0))}</b>")
            lines.append("</blockquote>")
            await query.message.answer("\n".join(lines))
        await query.answer()
        ...
        await query.message.answer("🛠️🎟 <b>Создание промо</b>\nШаг 1/3: введи код промо\nФормат: A-Z, 0-9, _ или - (3..24)")
        await query.answer()
    elif action == "refreshdb":
        init_db()
        await query.message.answer("✅ База данных обновлена (структура проверена).")
        await query.answer()
    elif action == "broadcast":
        await query.message.answer("📢 Введи сообщение для рассылки всем пользователям:")
        await state.set_state(AdminGiveStates.waiting_currency)
...
@dp.message(AdminGiveStates.waiting_currency)
async def admin_broadcast_input(message: Message, state: FSMContext):
    data = await state.get_data()
    if data.get("admin_action") == "broadcast":
        await state.clear()
        conn = get_db()
        try:
...
async def checks_create_cb(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.answer("Введи сумму на 1 активацию чека:")
    await query.answer()
...
async def checks_create_amount(message: Message, state: FSMContext):
    try:
        ...
        return
    if amount < 10:
        await message.answer("Минимум для одной активации: 10 dC")
        return
...
async def checks_create_count(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=check_share_kb(code)
    )

@dp.callback_query(F.data.startswith("check:copy:"))
async def check_copy_cb(query: CallbackQuery):
    code = query.data.split(":")[-1]
    await query.message.answer(f"🧾 Код чека: <code>{code}</code>")
    await query.answer("Код отправлен в чат!")
...
async def addpromo_command(message: Message):
    ...
    await message.answer(f"✅🎟 <b>Промокод сохранен</b>\n🔤 Код: <code>{code}</code>\n💰 Награда: <b>{fmt_money(reward)}</b>\n♾️ Активаций: <b>{activations}</b>")
...
async def new_promo_activations_input(message: Message, state: FSMContext):
    ...
    create_promo(code, reward, activations)
    await state.clear()
    await message.answer(f"✅🎉 <b>Промо создано</b>\n🔤 Код: <code>{code}</code>\n💰 Награда: <b>{fmt_money(reward)}</b>\n🎯 Активаций: <b>{activations}</b>\n🚀 Готово к использованию!")
...
async def bank_open_amount(message: Message, state: FSMContext):
    try:
        ...
        return
    if amount < 100:
        await message.answer("Минимальный депозит: 100 dC")
        return
    await state.update_data(amount=amount)
...
async def bank_term_cb(query: CallbackQuery, state: FSMContext):
    ...
    if not ok:
        ...
        await query.message.answer(f"✅ <b>Депозит открыт</b>\n<blockquote>Сумма: <b>{fmt_money(amount)}</b>\nСрок: <b>{term_days} дн.</b>\nДоходность: <b>+{int(rate * 100)}%</b></blockquote>")
...
async def bank_list_cb(query: CallbackQuery):
    ...
    for row in rows[:10]:
        ...
        status_text = "⏳ активен" if status == "active" and left > 0 else ("✅ можно снять" if status == "active" else f"✅ закрыт {fmt_dt(int(row['closed_at'] or opened_at))}")
...
async def bank_withdraw_cb(query: CallbackQuery):
    ...
    if closed == 0:
        ...
        await query.message.answer(f"✅ <b>Вывод депозитов</b>\nЗакрыто: <b>{closed}</b>\nНачислено: <b>{fmt_money(payout)}</b>")
        await query.message.answer(render_bank_panel_text(query.from_user.id), reply_markup=bank_kb())
    await query.answer()
...
async def games_command(message: Message):
    await message.answer("🎮 <b>Игры</b>\nВыбери игру кнопкой ниже.\nДальше вводи ставку прямо командой, например: <code>башня 300 2</code>.", reply_markup=games_kb())
...
async def games_pick_cb(query: CallbackQuery, state: FSMContext):
    ...
    usage_map = {"tower": "башня 300 2", "gold": "золото 300", "diamonds": "алмазы 300 2", "roulette": "рул 300 чет", "crash": "краш 300 2.5", "mines": "мины 300 3", "cube": "кубик 300 5", "dice": "кости 300 м", "ochko": "очко 300", "football": "футбол 300 гол", "basket": "баскет 300"}
    example = usage_map.get(game)
    if example:
        await query.message.answer(f"<i>Введи команду в чат:</i>\n<blockquote><code>{example}</code></blockquote>", parse_mode="HTML")
    await query.answer()

@dp.message(StateFilter(None), lambda m: normalize_text(m.text) in {"башня", "золото", "алмазы", "рулетка", "краш", "мины", "кубик", "кости", "очко", "футбол", "баскет"})
...
async def roulette_start(message: Message, state: FSMContext):
    ...
    if bet < MIN_BET:
        ...
        return
    choice_raw = parts[2]
    mapping = {"красное": "red", "кра": "red", "red": "red", "черное": "black", "чёрное": "black", "чер": "black", "black": "black", "чет": "even", "четное": "even", "чёт": "even", "чётное": "even", "even": "even", "нечет": "odd", "нечетное": "odd", "нечёт": "odd", "нечётное": "odd", "odd": "odd", "зеро": "zero", "zero": "zero", "зел": "zero", "0": "zero"}
    ...
    ok, new_balance = settle_instant_bet(user_id=user_id, bet=float(bet), payout=payout, choice=f"roulette:{choice}", outcome=outcome)
    if not ok:
        await message.answer("Недостаточно средств на балансе.")
        return
    await message.answer(
        f"{headline_user('🎡', user_id, message.from_user.first_name, 'рулетка сыграна')}\n"
        f"<blockquote>{outcome}\nСтавка: <b>{fmt_money(bet)}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(new_balance)}</b></blockquote>"
    )
...
async def roulette_amount(message: Message, state: FSMContext):
    ...
    await state.set_state(RouletteStates.waiting_choice)
    await message.answer("Выбери сектор или введи текстом: красное / черное / чет / нечет / зеро", reply_markup=roulette_choice_kb())
...
async def finish_roulette(message: Message, state: FSMContext, choice: str) -> None:
    ...
    win, multiplier, outcome = roulette_roll(choice)
    payout = round(bet * multiplier, 2) if win else 0.0
    ok, balance = settle_instant_bet(user_id=message.from_user.id, bet=bet, payout=payout, choice=f"roulette:{choice}", outcome=outcome)
    await state.clear()
    if not ok:
        await message.answer("Недостаточно средств на балансе.")
        return
...
async def roulette_choice_text(message: Message, state: FSMContext):
    ...
    mapping = {"красное": "red", "red": "red", "черное": "black", "чёрное": "black", "black": "black", "чет": "even", "четное": "even", "чёт": "even", "чётное": "even", "even": "even", "нечет": "odd", "нечетное": "odd", "нечёт": "odd", "нечётное": "odd", "odd": "odd", "зеро": "zero", "zero": "zero", "0": "zero"}
...
async def roulette_choice_cb(query: CallbackQuery, state: FSMContext):
    await finish_roulette(query.message, state, query.data.split(":")[-1])
    await query.answer()
...
async def crash_start(message: Message, state: FSMContext):
    ...
    if target < 1.01 or target > 10:
        ...
        return
    r = random.random()
    if r < 0.06: crash_multiplier = 1.00
    elif r < 0.55: crash_multiplier = round(random.uniform(1.01, 1.80), 2)
    elif r < 0.80: crash_multiplier = round(random.uniform(1.81, 2.80), 2)
    elif r < 0.93: crash_multiplier = round(random.uniform(2.81, 4.50), 2)
    elif r < 0.985: crash_multiplier = round(random.uniform(4.51, 9.50), 2)
    else: crash_multiplier = round(random.uniform(9.51, 10.0), 2)
    win = crash_multiplier >= target
    payout = round(bet * target, 2) if win else 0.0
    ok, new_balance = settle_instant_bet(user_id=user_id, bet=float(bet), payout=payout, choice=f"crash:{target}", outcome=f"crash={crash_multiplier}")
    if not ok:
        await message.answer("Недостаточно средств.")
        return
    await message.answer(
        f"📈 <b>Краш</b>: <b>{crash_multiplier}x</b>\nТвоя цель: <b>{target}x</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(new_balance)}</b>"
    )
...
async def crash_target(message: Message, state: FSMContext):
    ...
    win = target <= rolled
    payout = round(bet * target, 2) if win else 0.0
    ok, balance = settle_instant_bet(message.from_user.id, bet, payout, choice=f"crash:{target}", outcome=f"crash={rolled}")
    await state.clear()
    if not ok:
        await message.answer("Недостаточно средств.")
        return
...
async def cube_start(message: Message, state: FSMContext):
    ...
    ok, new_balance = settle_instant_bet(user_id=user_id, bet=float(bet), payout=payout, choice=f"cube:{bet_type}", outcome=f"num={number}")
    if not ok:
        await message.answer("Недостаточно средств.")
        return
    more_less = "меньше" if number <= 3 else "больше"
    parity = "чет" if number % 2 == 0 else "нечет"
    await message.answer(f"🎲 <b>Кубик</b>\nВыпало число: <b>{number}</b> ({more_less}, {parity})\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(new_balance)}</b>")
...
async def cube_guess(message: Message, state: FSMContext):
    ...
    try:
        ...
        if 1 <= guess <= 6:
            ...
            ok, balance = settle_instant_bet(message.from_user.id, bet, payout, choice=f"cube:{guess}", outcome=f"rolled={rolled}")
            await state.clear()
            if not ok:
                await message.answer("Недостаточно средств.")
                return
            await message.answer(f"{headline_user('🎲', message.from_user.id, message.from_user.first_name, 'кубик брошен')}\n<blockquote>Твой выбор: <b>{guess}</b>\nВыпало: <b>{rolled}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b></blockquote>")
            return
    ...
    payout = round(bet * mult, 2) if win else 0.0
    ok, balance = settle_instant_bet(message.from_user.id, bet, payout, choice=f"cube:{bet_type}", outcome=f"rolled={rolled}")
    await state.clear()
    if not ok:
        await message.answer("Недостаточно средств.")
        return
    await message.answer(f"{headline_user('🎲', message.from_user.id, message.from_user.first_name, 'кубик брошен')}\n<blockquote>Твой выбор: <b>{bet_type}</b>\nВыпало: <b>{rolled}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b></blockquote>")
...
async def dice_start(message: Message, state: FSMContext):
    ...
    payout = round(bet * mult, 2) if win else 0.0
    ok, new_balance = settle_instant_bet(user_id=user_id, bet=float(bet), payout=payout, choice=f"dice:{choice}", outcome=f"{d1}+{d2}={total}")
    if not ok:
        await message.answer("Недостаточно средств.")
        return
    relation = "меньше 7" if total < 7 else ("больше 7" if total > 7 else "равно 7")
    await message.answer(f"🎯 <b>Кости</b>\nВыпало: <b>{d1}</b> + <b>{d2}</b> = <b>{total}</b> ({relation})\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(new_balance)}</b>")
...
async def dice_guess(message: Message, state: FSMContext):
    ...
    payout = round(bet * mult, 2) if win else 0.0
    ok, balance = settle_instant_bet(message.from_user.id, bet, payout, choice=f"dice:{guess}", outcome=f"{d1}+{d2}={total}")
    await state.clear()
    if not ok:
        await message.answer("Недостаточно средств.")
        return
    await message.answer(f"{headline_user('🎯', message.from_user.id, message.from_user.first_name, 'кости брошены')}\n<blockquote>Кубики: <b>{d1}</b> и <b>{d2}</b> (сумма {total})\nТвой выбор: <b>{guess}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b></blockquote>")
...
async def football_start(message: Message, state: FSMContext):
    ...
    if choice:
        ...
        ok, new_balance = settle_instant_bet(user_id=user_id, bet=float(bet), payout=payout, choice=f"football:{choice}", outcome=f"result={outcome}")
        if not ok:
            await message.answer("Недостаточно средств.")
            return
        await message.answer(f"Итог: <b>{'Гол' if outcome == 'gol' else 'Мимо'}</b>\nТвой выбор: <b>{'Гол' if choice == 'gol' else 'Мимо'}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(new_balance)}</b>")
        return
    ...
    await message.answer(f"Футбол: выбери результат\nСтавка: <b>{fmt_money(bet)}</b>", reply_markup=kb)
...
async def football_amount(message: Message, state: FSMContext):
    ...
    balance = finalize_reserved_bet(message.from_user.id, bet, payout, choice="football", outcome=f"value={value}")
    await message.answer(f"{headline_user('⚽', message.from_user.id, message.from_user.first_name, 'матч сыгран')}\n<blockquote>Итог удара: <b>{result_title}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b></blockquote>")
...
async def football_cancel(query: CallbackQuery):
    ...
    await query.message.edit_text(f"Игра отменена. Возвращено: <b>{fmt_money(bet)}</b>\nБаланс: <b>{fmt_money(balance)}</b>")
    await query.answer()
...
async def football_play(query: CallbackQuery):
    ...
    win = outcome == choice
    payout = round(bet * FOOTBALL_MULTIPLIERS[choice], 2) if win else 0.0
    new_balance = finalize_reserved_bet(user_id=user_id, bet=float(bet), payout=payout, choice=f"football:{choice}", outcome=f"result={outcome}")
    NFOOTBALL_GAMES.pop(user_id, None)
    await query.message.answer(f"Итог: <b>{'Гол' if outcome == 'gol' else 'Мимо'}</b>\nТвой выбор: <b>{'Гол' if choice == 'gol' else 'Мимо'}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(new_balance)}</b>")
    await query.answer()
...
async def basket_start(message: Message, state: FSMContext):
    ...
    ok, new_balance = settle_instant_bet(user_id=user_id, bet=float(bet), payout=payout, choice="basketball", outcome=f"value={value}")
    if not ok:
        await message.answer("Недостаточно средств.")
        return
    await message.answer(f"Итог броска: <b>{result_title}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(new_balance)}</b>")
...
async def basket_amount(message: Message, state: FSMContext):
    ...
    result_title = basketball_value_text(value)
    payout = round(bet * 1.85, 2) if win else 0.0
    balance = finalize_reserved_bet(message.from_user.id, bet, payout, choice="basket", outcome=f"value={value}")
    await message.answer(f"{headline_user('🏀', message.from_user.id, message.from_user.first_name, 'бросок выполнен')}\n<blockquote>Итог броска: <b>{result_title}</b>\nРезультат: <b>{'Победа' if win else 'Поражение'}</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b></blockquote>")
...
def tower_text(game: Dict[str, Any]) -> str:
    ...
    return f"🗼 <b>Башня</b>\nСтавка: <b>{fmt_money(bet)}</b>\nЭтаж: <b>{level}</b>\nТекущий множитель: <b>x{current_mult:.2f}</b>\nПотенциально сейчас: <b>{fmt_money(current_win)}</b>\nСледующий этаж: <b>x{next_mult:.2f}</b>\n\nВыбери одну из 3 секций."
...
async def tower_pick(query: CallbackQuery):
    ...
    if chosen != safe:
        ...
        await query.message.edit_text(f"💥 <b>Башня</b>\nЛовушка в секции <b>{safe}</b>. Ты выбрал <b>{chosen}</b>.\nСтавка сгорела: <b>{fmt_money(bet)}</b>\nБаланс: <b>{fmt_money(balance)}</b>")
        await query.answer()
        return
    game["level"] += 1
...
async def tower_cash(query: CallbackQuery):
    ...
    if not game:
        await query.answer("Нет активной игры", show_alert=True)
        return
    bet, level = float(game["bet"]), int(game["level"])
    if level <= 0:
...
async def tower_cancel(query: CallbackQuery):
    ...
    if not game:
        ...
        return
    bet, level = float(game["bet"]), int(game["level"])
    payout = bet if level == 0 else 0.0
    outcome = "cancel_refund" if level == 0 else "cancel_lose"
...
def gold_text(game: Dict[str, Any]) -> str:
    ...
    return f"🥇 <b>Золото</b>\nСтавка: <b>{fmt_money(bet)}</b>\nРаунд: <b>{step}</b>\nТекущий множитель: <b>x{cur_mult:.2f}</b>\nПотенциально сейчас: <b>{fmt_money(cur_win)}</b>\nСледующий раунд: <b>x{next_mult:.2f}</b>\n\nВыбери плитку (одна из них с ловушкой)."
...
async def gold_start(message: Message, state: FSMContext):
    ...
    async with lock:
        ...
        gid = _new_gid("g")
        game = {"gid": gid, "uid": user_id, "stake": int(bet), "bad_cells": bad_cells, "current_level": 0, "path": [], "state": "playing"}
        ...
        await message.answer(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="❔", callback_data=f"ngold:{gid}:pick:0"), InlineKeyboardButton(text="❔", callback_data=f"ngold:{gid}:pick:1")],
...
async def legacy_gold_cb(query: CallbackQuery):
    ...
    if not game: return await query.answer("Игра завершена", show_alert=True)
    if game["uid"] != query.from_user.id: return await query.answer("Это не твоя игра", show_alert=True)
    lock = _game_lock(query.from_user.id)
    async with lock:
        game = NGOLD_GAMES.get(gid)
        if not game or game.get("state") != "playing": return await query.answer("Игра завершена", show_alert=True)
        if action == "cancel":
            if int(game["current_level"]) != 0: return await query.answer("Нельзя отменить после хода", show_alert=True)
            stake = int(game["stake"])
            balance = add_balance(query.from_user.id, stake)
            NGOLD_GAMES.pop(gid, None)
            await query.message.edit_text(f"Игра отменена. Возвращено: <b>{fmt_money(stake)}</b>\nБаланс: <b>{fmt_money(balance)}</b>")
            return await query.answer()
        if action == "collect":
            level = int(game["current_level"])
            if level <= 0: return await query.answer("Сделай хотя бы 1 ход", show_alert=True)
            mult = LEGACY_GOLD_MULTIPLIERS[level - 1]
            payout = int(round(int(game["stake"]) * mult))
            balance = finalize_reserved_bet(query.from_user.id, float(game["stake"]), float(payout), "gold", f"collect_lvl={level}")
            NGOLD_GAMES.pop(gid, None)
            await query.message.edit_text(f"Ты забрал приз: <b>{fmt_money(payout)}</b> (x{mult})\nБаланс: <b>{fmt_money(balance)}</b>")
            return await query.answer()
        if action == "pick":
            if choice not in {0, 1}: return await query.answer("Неверный выбор", show_alert=True)
            level = int(game["current_level"])
            if level >= len(LEGACY_GOLD_MULTIPLIERS): return await query.answer("Игра завершена", show_alert=True)
            bad = int(game["bad_cells"][level])
            game["path"].append(choice)
            if bad == choice:
                balance = finalize_reserved_bet(query.from_user.id, float(game["stake"]), 0.0, "gold", "lose")
                NGOLD_GAMES.pop(gid, None)
                await query.message.edit_text(f"Поражение.\nБаланс: <b>{fmt_money(balance)}</b>")
                return await query.answer()
            ...
            if game["current_level"] >= len(LEGACY_GOLD_MULTIPLIERS):
                ...
                balance = finalize_reserved_bet(query.from_user.id, float(game["stake"]), float(payout), "gold", "won_full")
                NGOLD_GAMES.pop(gid, None)
                await query.message.edit_text(f"Все уровни пройдены!\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b>")
                return await query.answer()
            NGOLD_GAMES[gid] = game
            stake, cur_level = int(game["stake"]), int(game["current_level"])
            rows = []
            for i in reversed(range(len(LEGACY_GOLD_MULTIPLIERS))):
                if i < len(game["path"]):
                    left, right = ("✅" if game["path"][i] == 0 else "◻️"), ("✅" if game["path"][i] == 1 else "◻️")
                else:
                    left = right = "❔"
                value = f"{fmt_money(int(round(stake * LEGACY_GOLD_MULTIPLIERS[i])))}"
                rows.append(f"|{left}|{right}| {value} ({LEGACY_GOLD_MULTIPLIERS[i]}x)")
            ...
            cur_amt, next_amt = int(round(stake * cur_mult)), int(round(stake * next_mult))
            await query.message.edit_text(
                "Золото: выбери клетку\n"
...
async def diamonds_start(message: Message, state: FSMContext):
    ...
    async with lock:
        ...
        for _ in range(ND_TOTAL_ROWS):
            ...
            bombs.append(row)
        gid = _new_gid("d")
        state_data = {"gid": gid, "uid": user_id, "bet": int(bet), "mines_amount": int(mines_amount), "level": 0, "bombs": bombs, "selected": [], "lost": False, "state": "playing", "multipliers_history": []}
        NDIAMOND_GAMES[gid] = state_data
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❔", callback_data=f"ndiam:{gid}:pick:0"), InlineKeyboardButton(text="❔", callback_data=f"ndiam:{gid}:pick:1"), InlineKeyboardButton(text="❔", callback_data=f"ndiam:{gid}:pick:2")],
            [InlineKeyboardButton(text="Отмена", callback_data=f"ndiam:{gid}:cancel")],
        ])
        await message.answer(f"Алмазы: выбери ячейку\nСтавка: <b>{fmt_money(bet)}</b>\nМин в ряду: <b>{mines_amount}</b>", reply_markup=kb)
...
async def legacy_diamonds_cb(query: CallbackQuery):
    ...
    if not state_data: return await query.answer("Игра завершена", show_alert=True)
    if int(state_data["uid"]) != query.from_user.id: return await query.answer("Это не твоя игра", show_alert=True)
    lock = _game_lock(query.from_user.id)
    async with lock:
        state_data = NDIAMOND_GAMES.get(gid)
        if not state_data or state_data.get("state") != "playing": return await query.answer("Игра завершена", show_alert=True)
        bet = int(state_data["bet"])
        ND_TOTAL_ROWS, ND_COLUMNS, ND_HOUSE_EDGE = 16, 3, 0.985
        if action == "cancel":
            if int(state_data["level"]) != 0 or state_data["selected"]: return await query.answer("После первого хода отмена недоступна", show_alert=True)
            balance = add_balance(query.from_user.id, bet)
            NDIAMOND_GAMES.pop(gid, None)
            await query.message.edit_text(f"Игра отменена. Возвращено: <b>{fmt_money(bet)}</b>\nБаланс: <b>{fmt_money(balance)}</b>")
            return await query.answer()
        if action == "collect":
            if int(state_data["level"]) == 0: return await query.answer("Сделай хотя бы 1 ход", show_alert=True)
            prod = 1.0
            for m in state_data.get("multipliers_history", []): prod *= m
            total_mult = round(prod, 6)
            payout = int(round(bet * total_mult))
            state_data["state"] = "collected"
            balance = finalize_reserved_bet(query.from_user.id, float(bet), float(payout), "diamonds", "collect")
            NDIAMOND_GAMES.pop(gid, None)
            await query.message.edit_text(f"Приз забран: <b>{fmt_money(payout)}</b> (x{round(total_mult, 2)})\nБаланс: <b>{fmt_money(balance)}</b>")
            return await query.answer()
        if action == "pick":
            if idx is None or idx < 0 or idx >= ND_COLUMNS: return await query.answer("Неверный выбор", show_alert=True)
            level = int(state_data["level"])
            if level < 0 or level >= ND_TOTAL_ROWS: return await query.answer("Неверный уровень", show_alert=True)
            ...
            step_mult = round((1.0 / p_safe) * ND_HOUSE_EDGE, 6) if p_safe > 0 else 1.0
            if step_mult <= 0: step_mult = 1.0
            state_data["selected"].append(idx)
            if state_data["bombs"][level][idx] == 1:
                state_data["lost"], state_data["state"] = True, "lost"
                balance = finalize_reserved_bet(query.from_user.id, float(bet), 0.0, "diamonds", "explode")
                NDIAMOND_GAMES.pop(gid, None)
                await query.message.edit_text(f"Ты попал на мину.\nРяд: <b>{level + 1}</b>\nБаланс: <b>{fmt_money(balance)}</b>")
                return await query.answer()
            ...
            if int(state_data["level"]) >= ND_TOTAL_ROWS:
                ...
                balance = finalize_reserved_bet(query.from_user.id, float(bet), float(payout), "diamonds", "won_full")
                NDIAMOND_GAMES.pop(gid, None)
                await query.message.edit_text(f"Все ряды пройдены.\nВыплата: <b>{fmt_money(payout)}</b> (x{round(total_mult, 2)})\nБаланс: <b>{fmt_money(balance)}</b>")
                return await query.answer()
            prod = 1.0
            for m in state_data.get("multipliers_history", []): prod *= m
            total_mult = round(prod, 6)
            ...
            for i in range(start_prev, cur_level):
                ...
                kb_rows.append(r)
            kb_rows.append([InlineKeyboardButton(text="❔", callback_data=f"ndiam:{gid}:pick:{j}") for j in range(ND_COLUMNS)])
            kb_rows.append([InlineKeyboardButton(text="Забрать", callback_data=f"ndiam:{gid}:collect")])
            await query.message.edit_text(
                f"Алмазы: игра продолжается\nРяд: <b>{cur_level}/{ND_TOTAL_ROWS}</b>\nМин в ряду: <b>{state_data['mines_amount']}</b>\n"
                f"Текущий множитель: <b>x{round(total_mult, 2)}</b>\nВозможный приз: <b>{fmt_money(potential)}</b>",
...
def mines_text(game: Dict[str, Any]) -> str:
    bet = float(game["bet"])
    opened_count, mines_count = len(game["opened"]), int(game["mines_count"])
    mult = mines_multiplier(opened_count, mines_count)
    potential = round(bet * mult, 2)
    return f"💣 <b>Мины</b>\n<blockquote>Ставка: <b>{fmt_money(bet)}</b>\nМин: <b>{mines_count}</b>\nОткрыто безопасных: <b>{opened_count}</b>\nТекущий множитель: <b>x{mult:.2f}</b>\nПотенциально сейчас: <b>{fmt_money(potential)}</b></blockquote>\n\n<i>Открывай клетки или забирай выигрыш.</i>"
...
async def mines_cell(query: CallbackQuery):
    ...
    if idx in game["mines"]:
        ...
        await query.message.edit_text(f"💥 <b>Мины</b>\n<blockquote>Ты попал на мину в клетке <b>{idx}</b>.\nПотеряно: <b>{fmt_money(bet)}</b>\nБаланс: <b>{fmt_money(balance)}</b></blockquote>", reply_markup=mines_kb(game, reveal_all=True))
        MINES_GAMES.pop(user_id, None)
        await query.answer()
        return
    game["opened"].add(idx)
    safe_opened, safe_total = len(game["opened"]), 9 - int(game["mines_count"])
...
async def mines_cash(query: CallbackQuery):
    ...
    if not game:
        await query.answer("Нет активной игры", show_alert=True)
        return
    bet, safe_opened, mines_count = float(game["bet"]), len(game["opened"]), int(game["mines_count"])
    if safe_opened <= 0:
...
async def mines_cancel(query: CallbackQuery):
    ...
    if safe_opened <= 0:
        ...
        payout, outcome = round(bet * mines_multiplier(safe_opened, mines_count), 2), f"cancel_cashout_{safe_opened}"
    balance = finalize_reserved_bet(user_id, bet, payout, "mines", outcome)
    await query.message.edit_text(f"❌ <b>Мины завершены</b>\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b>", reply_markup=mines_kb(game, reveal_all=True))
    MINES_GAMES.pop(user_id, None)
    await query.answer()
...
async def ochko_start(message: Message, state: FSMContext):
    ...
    async with lock:
        ...
        await message.answer(f"{headline_user('🎴', message.from_user.id, message.from_user.first_name, 'желаешь начать игру?')}\n<blockquote>Ставка: <b>{fmt_money(float(bet))}</b>\n<i>После начала игры отменить ее уже нельзя.</i></blockquote>", reply_markup=ochko_confirm_kb())
...
async def ochko_cancel_before_start(query: CallbackQuery, state: FSMContext):
    await state.clear()
    await query.message.edit_text(f"{headline_user('❌', query.from_user.id, query.from_user.first_name, 'игра в очко отменена')}\n<blockquote><i>Ставка не списана.</i></blockquote>")
    await query.answer()
...
async def ochko_start_confirm(query: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    if not ok:
        await query.message.edit_text(f"{headline_user('❌', query.from_user.id, query.from_user.first_name, 'недостаточно средств для игры')}")
        await query.answer()
        return
    deck = make_deck()
    ...
    if player_value == 21:
        if dealer_value == 21:
            payout, outcome = bet, "blackjack_push"
            text = f"🎴 <b>Очко</b>\n{render_ochko_table(game, reveal_dealer=True)}\n\nНичья по blackjack."
        else:
            payout, outcome = round(bet * 2.5, 2), "blackjack_win"
            text = f"🎴 <b>Очко</b>\n{render_ochko_table(game, reveal_dealer=True)}\n\nBlackjack!"
        balance = finalize_reserved_bet(query.from_user.id, bet, payout, "ochko", outcome)
        OCHKO_GAMES.pop(query.from_user.id, None)
        await query.message.edit_text(f"{headline_user('🎴', query.from_user.id, query.from_user.first_name, 'игра окончена')}\n{text}\nВыплата: <b>{fmt_money(payout)}</b>\nБаланс: <b>{fmt_money(balance)}</b>")
        await query.answer()
        return
...
async def ochko_stand(query: CallbackQuery):
    ...
    dealer_value, player_value = hand_value(game["dealer"]), hand_value(game["player"])
    bet = float(game["bet"])
    if dealer_value > 21 or player_value > dealer_value:
        payout, result_text, outcome = round(bet * 2.0, 2), "Победа", "win"
    elif dealer_value == player_value:
        payout, result_text, outcome = round(bet, 2), "Ничья", "push"
    else:
        payout, result_text, outcome = 0.0, "Поражение", "lose"
...
async def legacy_tower_cb(query: CallbackQuery):
    ...
    if not game: return await query.answer("Игра завершена", show_alert=True)
    if int(game["uid"]) != query.from_user.id: return await query.answer("Это не твоя игра", show_alert=True)
    lock = _game_lock(query.from_user.id)
    async with lock:
        game = NTOWER_GAMES.get(gid)
        if not game or game.get("state") != "playing": return await query.answer("Игра завершена", show_alert=True)
        if action == "cancel":
            if int(game["level"]) != 0 or game["selected"]: return await query.answer("После первого хода отмена недоступна", show_alert=True)
            bet = int(game["bet"])
            balance = add_balance(query.from_user.id, bet)
        ...
        if action == "collect":
            ...
            balance = finalize_reserved_bet(query.from_user.id, float(game["bet"]), float(payout), "tower", f"collect_lvl={level}")
            NTOWER_GAMES.pop(gid, None)
            await query.message.edit_text(f"Приз забран: <b>{fmt_money(payout)}</b> (x{mult})\nБаланс: <b>{fmt_money(balance)}</b>")
            return await query.answer()
        if action == "pick":
            if choice is None or choice < 0 or choice > 4: return await query.answer("Неверный выбор", show_alert=True)
            level = int(game["level"])
            if level < 0 or level > 8: return await query.answer("Неверный уровень", show_alert=True)
            game["selected"].append(choice)
            if game["bombs"][level][choice] == 1:
                game["state"] = "lost"
                balance = finalize_reserved_bet(query.from_user.id, float(game["bet"]), 0.0, "tower", "lose")
                NTOWER_GAMES.pop(gid, None)
                await query.message.edit_text(f"Поражение.\nУровень: <b>{level}/9</b>\nБаланс: <b>{fmt_money(balance)}</b>")
                return await query.answer()
            game["level"] = level + 1
            if int(game["level"]) >= 9:
                ...
                payout = int(round(int(game["bet"]) * mult))
                game["state"] = "won"
                balance = finalize_reserved_bet(query.from_user.id, float(game["bet"]), float(payout), "tower", "won_top")
                NTOWER_GAMES.pop(gid, None)
                await query.message.edit_text(f"Вершина пройдена!\nВыплата: <b>{fmt_money(payout)}</b> (x{mult})\nБаланс: <b>{fmt_money(balance)}</b>")
                return await query.answer()
            NTOWER_GAMES[gid] = game
            cur_level, selected = int(game["level"]), game["selected"]
            kb_rows = [[InlineKeyboardButton(text="❔", callback_data=f"ntower:{gid}:pick:{j}") for j in range(5)]]
            for i in range(cur_level - 1, -1, -1):
                chosen = selected[i] if i < len(selected) else None
                kb_rows.append([InlineKeyboardButton(text=("✅" if chosen == j else "◻️"), callback_data="nnoop") for j in range(5)])
            ...
            await query.message.edit_text(
                f"Башня\nРяд: <b>{min(cur_level + 1, 9)}/9</b>\nСтавка: <b>{fmt_money(int(game['bet']))}</b>\nМин в ряду: <b>{game['mines']}</b>\n"
                f"Текущий множитель: <b>x{now_mult}</b>\nСледующий множитель: <b>x{next_mult}</b>\nПотенциальный приз: <b>{fmt_money(int(int(game['bet']) * next_mult))}</b>",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows),
            )
            return await query.answer()
    await query.answer()
